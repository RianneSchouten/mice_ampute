...
We present an R-function to generate missing values in complete datasets. Such an amputation procedure is useful to accurately evaluate the effect of missing data on analysis outcomes. R-function [`ampute`](https://www.rdocumentation.org/packages/mice/versions/3.9.0/topics/ampute) is available in multiple imputation package [**mice**](https://github.com/stefvanbuuren/mice). [Van Buuren's book [-@Stef2018]](https://stefvanbuuren.name/fimd/ch-introduction.html) gives an extensive overview of missing data methodology and multiple imputation algorithm MICE. In this tutorial, we will focus on amputation, which is **the generation of missing values in complete data** and as such, the opposite of imputation. 
...
For a theoretical justification and a demonstration of the method, we refer to [Schouten, Lugtig and Vink [-@Schouten2018a]](https://doi.org/10.1080/00949655.2018.1491577). The paper discusses how missing data methods are evaluated in four steps:
...
An example of how `ampute` can be used to evaluate missing data methods can be found in [Schouten and Vink [-@Schouten2018b]](https://doi.org/10.1177/0049124118799376). With `ampute` it is straightforward to generate missing values in multivariate datasets, with any desired proportion, varying underlying mechanisms, different missingness patterns and varying data distributions.
...
One of those factors is the missingness **mechanism**. In case of MCAR missingness, all data rows have the same probability of being amputed. For MAR and MNAR missingness, we will use so-called **weighted sum scores**. In essence, MAR misingness occurs when the information about the missing data is in the observed data. In case of MNAR missingness, the information about the missing data is missing itself. For a discussion of how these mechanisms interact with observed data, see [Schouten and Vink [-@Schouten2018b]](https://doi.org/10.1177/0049124118799376).
...
Use the help function to obtain `ampute`'s [documentation](https://www.rdocumentation.org/packages/mice/versions/3.9.0/topics/ampute). The function is available in multiple imputation package [**mice**](https://github.com/stefvanbuuren/mice). 
...
We will now decide what kind of missingness **mechanism** we will implement. As said before, there are three mechanisms: MCAR, MAR and MNAR. In case of MCAR missingness, all data rows have the same probability of being amputed. With MAR misingness, the information about the missing data is in the observed data and with MNAR missingness, the information about the missing data is missing itself. We refer to [Schouten and Vink [-@Schouten2018b]](https://doi.org/10.1177/0049124118799376) for a more thorough discussion of missingness mechanisms.
