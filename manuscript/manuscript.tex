...
\renewcommand{\refname}{Word Count: 6345}
...
\maketitle
\author{
\name{R.~M. Schouten\textsuperscript{a, b}\thanks{CONTACT R.~M. Schouten. E-mail: r.m.schouten@uu.nl} and Peter Lugtig\textsuperscript{a} and Gerko Vink\textsuperscript{a}}
\affil{\textsuperscript{a}\small University of Utrecht, Department of Methodology and Statistics, Sjoerd Groenman building, Padualaan 14, 3584 CH Utrecht, The Netherlands; \textsuperscript{b}\small DPA Professionals, Business Unit DPA IT, Team Data Science, Gatwickstraat 11, 1043 GL Amsterdam, The Netherlands}
}
...
\noindent[Table 3 about here.]

\noindent[Table 4 about here.]
...
\begin{table}
\tbl{Five simulation conditions for the evaluation of \code{ampute}}
...
weak mnar & & 0 & 0 & 1 && 1 & 0 & 5\\
   \hline
   \end{tabular}}
\tabnote{\textit{Note.} The pattern column displays whether variables are amputed ($0$) or remain complete ($1$). The weights column displays whether the variables influence the amputations.}
\label{cond1}
\end{table}

\begin{table}
\tbl{Generation of MAR missingness on 2 variables with standard and shifted stepwise univariate amputation (SUA) and multivariate amputation (MA)}
...
\begin{table}
\tbl{Generation of MAR missingness on 3 variables with standard and shifted stepwise univariate amputation (SUA) and multivariate amputation (MA)}
{\begin{tabular}{rrrrrrrrrrrr}
  \hline
&& \multicolumn{2}{c}{\%mis} && \multicolumn{3}{c}{cca} && \multicolumn{3}{c}{mi} \\
\cline{3-4} \cline{6-8} \cline{10-12}
...
\tabnote{\textit{Note.} Estimates of $\mathbb{E}_{Y_1}$ are evaluated with complete case analysis (cca) and multiple imputation (mi). Comparisons are done for the intended (int) and obtained (obt) missingness percentage (\%mis), the bias, confidence interval width (ciw) and coverage rate (cov) for correlations $\rho \in \{0.2, 0.5, 0.8\}$ (cor).}
  ...
  \begin{table}
\tbl{Evaluation of R-function \code{ampute}}
...
\hline
 & 0.1 & -0.000 & 0.139 & 0.955 &  & -0.001 & 0.145 & 0.956 \\ 
  ...
  weak mnar & 0.5 & -0.109 & 0.136 & 0.123 &  & -0.028 & 0.140 & 0.877 \\ 
   ...
   \hline
\end{tabular}}
\tabnote{\textit{Note.} Bias, confidence interval width (ciw) and coverage rate (cov) of $\mathbb{E}_{Y_1}$ estimates are shown for complete case analysis (cca) and multiple imputation by predictive mean matching (mi). Data correlations are $\rho \in \{0.1, 0.2, \dots, 0.9\}$ (cor).}
  \label{A}
\end{table}

\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{types_shifted.eps}
...
\hspace*{-1cm}
\includegraphics[width =1.0\linewidth]{Scheme_large.eps}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{types.eps}
\caption{Four variants of the logistic distribution function \citep[based on][p. 64]{Stef2012}}
\label{types}
\end{figure}

\begin{figure}
\subfigure[]{\resizebox*{.51\textwidth}{!}{\includegraphics[width = 1.0\linewidth]{Figure1a.eps}}\label{01a}}\hspace{5pt}
\subfigure[]{\resizebox*{.51\textwidth}{!}{\includegraphics[width = 1.0\linewidth]{Figure1b.eps}}\label{01b}}\hspace{5pt}
\subfigure[]{\resizebox*{.51\textwidth}{!}{\includegraphics[width = 1.0\linewidth]{Figure1c.eps}}\label{01c}}\hspace{5pt}
\subfigure[]{\resizebox*{.51\textwidth}{!}{\includegraphics[width = 1.0\linewidth]{Figure1d.eps}}\label{01d}}\hspace{5pt}
\subfigure[]{\resizebox*{.51\textwidth}{!}{\includegraphics[width = 1.0\linewidth]{Figure1e.eps}}\label{01e}}\hspace{5pt}
\subfigure[]{\resizebox*{.51\textwidth}{!}{\includegraphics[width = 1.0\linewidth]{Figure1f.eps}}\label{01f}}\hspace{5pt}
